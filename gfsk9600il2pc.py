...
from modems_codecs.gfsk import GFSKModem
...
def main():
	...
	modems = []
	modems.append(GFSKModem(sample_rate=input_sample_rate, config='9600'))
	modems.append(GFSKModem(sample_rate=input_sample_rate, config='9600 inverted'))
	...
	for demod_audio in demod_audios:
		slicers.append(BinarySlicer(sample_rate=input_sample_rate, config='9600'))
	...
	for sliced_data in sliced_datas:
		il2p_codecs.append(IL2PCodec(ident=i, crc=True, min_dist=0))
		i += 1
	...
	results.CalcCRCs()
	results.Correlate(address_distance=input_sample_rate/16)
